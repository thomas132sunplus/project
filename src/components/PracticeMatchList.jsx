          ...
          try {
            await createPracticeMatch(
                  ...
                  for (const match of filteredEnriched) {
                    if (!match.tournamentData || !match.tournamentData.name) continue;
                    ...
                    {Object.keys(groupedMatches).length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {Object.entries(groupedMatches)
                          .filter(([, data]) => data.tournament && data.tournament.name)
                          .map(([tournamentId, data]) => (
                            <button
                              key={tournamentId}
                            ...
                            >
                              <h3 className="text-xl font-bold text-gray-800 mb-3">{data.tournament.name}</h3>
                              <div className="mb-4">
              ...
              // 若選到的盃賽資料異常（無 name），直接返回列表
              if (!currentTournamentData?.tournament?.name) {
                return (
                  <div className="container mx-auto px-4 py-8">
                    ...
                    <div className="bg-red-50 border-l-4 border-red-400 p-6 rounded">
                      <p className="text-red-800">此盃賽資料異常，請重新選擇。</p>
                    </div>
                  </div>
                );
              }
              const tournamentMatches = (currentTournamentData?.matches || []).filter(m => m.tournamentData && m.tournamentData.name);
              return (
                <div className="container mx-auto px-4 py-8">
                  <button onClick={handleBackToList} className="mb-4 text-blue-600 hover:underline flex items-center">← 返回盃賽列表</button>
                  <div className="mb-8">
                    <h1 className="text-3xl font-bold text-gray-800 mb-2">{currentTournamentData.tournament.name}</h1>
